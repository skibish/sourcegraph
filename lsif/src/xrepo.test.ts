...
describe('XrepoDatabase', () => {
    ...
    it('should not return elements farther than MAX_TRAVERSAL_LIMIT', async () => {
        ...
        expect(await xrepoDatabase.findClosestCommitWithData('foo', '-1')).toEqual('0')
        expect(await xrepoDatabase.findClosestCommitWithData('foo', `${+(limit - 1)}`)).toEqual('0')
        expect(await xrepoDatabase.findClosestCommitWithData('foo', `${-(limit - 1)}`)).toEqual('0')
        expect(await xrepoDatabase.findClosestCommitWithData('foo', `${+(limit - 2)}`)).toEqual('0')
        expect(await xrepoDatabase.findClosestCommitWithData('foo', `${-(limit - 2)}`)).toEqual('0')
        expect(await xrepoDatabase.findClosestCommitWithData('foo', `${+(limit + 0)}`)).toBeUndefined()
        expect(await xrepoDatabase.findClosestCommitWithData('foo', `${-(limit + 0)}`)).toBeUndefined()
        expect(await xrepoDatabase.findClosestCommitWithData('foo', `${+(limit + 1)}`)).toBeUndefined()
        expect(await xrepoDatabase.findClosestCommitWithData('foo', `${-(limit + 1)}`)).toBeUndefined()
        expect(await xrepoDatabase.findClosestCommitWithData('foo', `${+(limit + 2)}`)).toBeUndefined()
        expect(await xrepoDatabase.findClosestCommitWithData('foo', `${-(limit + 2)}`)).toBeUndefined()

        // Modify markers, re-test extreme bounds
        await xrepoDatabase.addPackagesAndReferences('foo', '1', [], [])
        expect(await xrepoDatabase.findClosestCommitWithData('foo', `${+limit}`)).toEqual('1')
        expect(await xrepoDatabase.findClosestCommitWithData('foo', `${-limit}`)).toBeUndefined()
    })
})
