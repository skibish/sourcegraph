        ...
        return this.withConnection(async connection => {
            const query = `
                ...
                with recursive ancestors(repository, "commit", parent_commit, has_lsif_data) as (
                    select c.* from lsif_commits_with_lsif_data c where c.repository = $1 and c."commit" = $2
                    union
                ...
                select t."commit" from (
                    select * from (select a.*, row_number() over () as n from ancestors a limit $3) as f
                    union
                    select * from (select d.*, row_number() over () as n from descendants d limit $3) as q
                ) t
